...
//|                                                                  |
//|  對應回測：fx_engine_v2.py / fx_portfolio_v2.py                   |
//|  驗證績效（2.78年真實歷史、真實點差）：                            |
...
#property description "InpLotMultiplier 可整體放大/縮小下單量（如設 2 = 兩倍），總曝險上限不受此影響。"
...
int OnInit()
{
   ...
   PrintFormat("✅ EA 啟動：%d 檔標的，H4 週期，EMA%d / ATR%d，最多 %d 層 DCA",
               g_count, InpEmaPeriod, InpAtrPeriod, InpMaxLayers);
...
void OnTick()
{
   ...
   string dashboard = StringFormat("通道網格 DCA ─ 淨值 %.2f ─ 首單名目 %.0f（下單倍數 x%.2f）\n",
